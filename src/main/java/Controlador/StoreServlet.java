    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            for (int i = 0; i < productList.size(); i++){
                if (productList.get(i).getType() == 0)
                    frameList.add(productList.get(i));
                if (productList.get(i).getType() == 1)
                    animFrameList.add(productList.get(i));
                if (productList.get(i).getType() == 2)
                    picList.add(productList.get(i));
            }
            request.setAttribute("frameList", frameList);
            request.setAttribute("animFrameList", animFrameList); 
            request.setAttribute("picList", picList); 
            
           
            
            RequestDispatcher dispatcher;
            dispatcher = getServletContext().getRequestDispatcher("/StoreDynamic.jsp");
            dispatcher.forward(request, response);
